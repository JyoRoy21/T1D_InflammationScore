### T1D Disease Perturbations (doen)
## Antonio Holmes
# 09/03/2024
...
down_gene_dataset<- gene_expected_count %>% filter(str_detect(external_gene_name, paste(row_as_column_df$Genes, collapse = "|"))) %>% filter(entrezgene_id!=".")
  #gene_expected_count %>% filter(external_gene_name%in%row_as_column_df$Genes)

# Creating Matrix --------------------------------------------------------------

# The genes I am interested in (HAS TO BE ENTREZ ID)
genes_of_interest<- down_gene_dataset$entrezgene_id
