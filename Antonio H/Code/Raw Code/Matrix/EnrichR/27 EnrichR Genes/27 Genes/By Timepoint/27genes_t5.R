...
#
source("Code/Raw Code/Matrix/EnrichR/27 EnrichR Genes/27 Genes/27 genes heatmap.R")

t5_dataset <- gene_expected_count %>% select(entrezgene_id,external_gene_name,contains("_t5"))

# Matrix & Plotting ----------------------------------------------------------------

#Required Matrix & changes 
gene_expected_matrix <- t5_dataset %>%
